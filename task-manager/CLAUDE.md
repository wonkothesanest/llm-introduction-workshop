...
- `Priority` type: Union type for urgency levels (low, medium, high)
...
**Endpoints:**
- `GET /api/tasks` - List all troubleshooting steps, optional ?status and ?priority filters
- `GET /api/tasks/:id` - Get single step details
- `POST /api/tasks` - Create new custom troubleshooting step
- `PUT /api/tasks/:id` - Update step (mark complete, change priority, add notes)
- `DELETE /api/tasks/:id` - Remove step

**Sample Troubleshooting Steps:**
1. Check main solar breaker (electrical, high priority)
2. Verify inverter display is on (inverter, high priority)
3. Confirm internet connectivity (connectivity, medium priority)
4. Check for physical damage or debris (physical-inspection, low priority)
...
- Supports filtering by both status and priority
...
- Handles priority filtering (show only specific priority levels)
...
- Priority badge with color coding (high=red, medium=orange, low=green)
...
3. **Support Agent Creates Custom Step:**
   ...
   - Backend creates step with "todo" status and selected priority → Returns new Task
   - TaskList reloads, new step appears for homeowner

4. **Filtering by Status and Priority:**
   - User selects filters in TaskList → State updated
   - useEffect triggers → Calls API with ?status and ?priority parameters
...
- **Priority levels:** High priority = safety or complete outage; Medium = reduced performance; Low = optimization or monitoring

### Common Troubleshooting Patterns

**No Production Data:**
1. Check breakers (most common issue) - HIGH priority
2. Verify inverter is on and displaying properly - HIGH priority
3. Confirm internet connectivity for monitoring - MEDIUM priority
4. Check for error codes on inverter display - HIGH priority

**Low Production:**
1. Check for physical obstructions (snow, debris, shading) - MEDIUM priority
2. Verify all panels are clean - LOW priority
3. Check for error indicators - HIGH priority
4. Compare to expected production (seasonal adjustments) - LOW priority

**Monitoring Offline:**
1. Verify home internet is working - MEDIUM priority
2. Check if monitoring device has power/lights - MEDIUM priority
3. Restart monitoring equipment (power cycle) - MEDIUM priority
4. Verify credentials still work in portal - LOW priority
...
- Type aliases for unions (TaskStatus, Priority, TaskCategory)
...
- Types/Interfaces: PascalCase (Task, TaskStatus, Priority)
...
- Class names: kebab-case (.task-item, .status-done, .priority-high)
...
**Unit Tests:**
- Service layer methods (taskService.test.ts)
...
- Conditional rendering (filters, error states)
- Prop handling and callbacks
- Accessibility requirements

**Test Patterns:**
- Arrange: Set up mocks and test data
- Act: Trigger user action or function call
...
5. Provide expected outcome (what should they see if it's working)
6. Consider categorizing with TaskCategory for future filtering

### Following Existing Patterns
When adding features:
- Look at how status filtering is implemented
- Match the UI patterns for dropdowns and filters
- Follow the same API parameter style (?status=done → ?priority=high)
...
*This documentation reflects the workshop completion state with priority feature and test coverage.*
